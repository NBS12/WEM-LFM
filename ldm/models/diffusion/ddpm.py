...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        # Gate Fusion model for concatenating Mass Mask CLIP embedding and Mass Radiomic Features 
        self.cc_projection = GateFusionModel(m=5,n=5,k=5)
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        x = batch['image_target'].to(memory_format=torch.contiguous_format).float().to(self.device)
        xc = batch['image_cond'].to(memory_format=torch.contiguous_format).float().to(self.device)
        mass= batch['mass_cond'].to(memory_format=torch.contiguous_format).float().to(self.device)
        features = batch['additional_feature'].to(memory_format=torch.contiguous_format).float().to(self.device)
        ...
        # To support classifier-free guidance, randomly drop out only mass feature 5%, only mask conditioning 5%, and both 5%.
        random = torch.rand(x.size(0), device=x.device)
        prompt_mask = 1 - rearrange((random < 2 * uncond).float(), "n -> n 1 1")
        input_mask = 1 - rearrange((random >= uncond).float() * (random < 3 * uncond).float(), "n -> n 1 1 1")

        # z.shape: [8, 4, 64, 64]; c.shape: [8, 1, 768]
        # print('=========== xc shape ===========', xc.shape)
        with torch.enable_grad():
            clip_emb = self.get_learned_conditioning(mass).detach()
            c_crossattn = self.cc_projection(clip_emb, features)
